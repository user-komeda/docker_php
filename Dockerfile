...
RUN export PKG_CONFIG_PATH="$PKG_CONFIG_PATH:/usr/local/libzip/lib64/pkgconfig" && cd php-$PHP_VERSION && ./configure --prefix=/usr/local/ --enable-sockets --with-openssl --with-zip --with-pdo --with-pdo-mysql && make -j$(getconf _NPROCESSORS_ONLN) && make install
